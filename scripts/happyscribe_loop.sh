...
#   STUDY=pundits nohup bash scripts/happyscribe_loop.sh > data-pundits/logs/happyscribe_loop.log 2>&1 &
set -uo pipefail
cd "$(dirname "$0")/.."

# Which study this loop fetches for, and where its data lives. The dedupe calls
# below pass no path flags, so they take their paths from the exported STUDY.
