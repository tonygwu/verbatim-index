...
| VP-18 | The live board tells readers a lead-time rule that is no longer the rule | **open**, found by repo-1 2026-09-16, NOT fixed here because `build_predictions_site.py` is repo-2's file and its session has ended. `score_why()` hand-types "reach at least six months out" while `MIN_LEAD_DAYS` has been 60 since the operator changed it from 180 on 2026-09-15. Six months is 180 days, so the page states the OLD floor. That string IS rendered: it is the `n_scored == 0` branch, and line 425 shows `score_why` as the tooltip on exactly the rows with no score. 10 of the 40 live rows carry it. Fix is to derive the number from the constant rather than type it, which is the rule this repo already holds itself to for judge lists and test globs | `curl -s https://verbatim-predictions.tonygwu.com/ \| grep -o 'six months out'`; then `grep -n '^MIN_LEAD_DAYS' scripts/phase2_resolvability.py` |
