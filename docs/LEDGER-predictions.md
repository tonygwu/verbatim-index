...
| VP-16 | 8 past-due predictions target a date BEFORE their own statement date | **open, and now priced.** 5 leaders, 2.5% of the 325 dated records; cause is the upload date standing in for the date of speech, so a 2006 talk uploaded in 2013 resolves "this year" to the wrong year. The detector only catches the provably-impossible direction. MEASURED 2026-09-16 against the published board, which is what makes this worth scheduling rather than merely recording: `deadline_incoherent` is costing **7 records across leaders still UNDER the rank floor**, and it is concentrated. bill-gates carries 4 of them and needs only 2 more scored predictions to rank, so this defect alone may be the whole of why he has no number. vlad-tenev carries 2 and arvind-krishna 1. Repair means recovering each recording's true date and is repo-0's, since it edits production records. It is the cheapest unscored-to-scored lever found in round 2 that needs no new sourcing and no discovery quota, only the dates | `.venv/bin/python scripts/phase2_resolvability.py --as-of $(date -u +%F) \| tail -12`; per-leader blast radius from `data/predictions/scores.json` by counting `not_scored_because == 'unresolvable:deadline_incoherent'` for leaders with `n_scored < 3` |
...
| VP-21 | Round 2 placed and the board redeployed | **succeeded 2026-09-16, and this row exists because repo-1 has no other way to see the outcome: it works from its own data branch `codex/repo-1-data-2026-09-14`, not production main.** Both supplemental rounds were placed into production `data/predictions/<slug>/` and `data/transcripts_web/<slug>/`, validated 232 -> 0 failures, and committed to `verbatim-index-data` main at `ba86d83b`. Twelve run manifests had to be copied alongside the records; placing records without them produced 432 provenance failures, so a manifest is part of the payload and not an artefact of the run. RE-DERIVED against the live page 2026-09-16, not taken from the deploy log: the page carries 45 distinct slugs and 45 `n_scored` entries, 28 of them at or above the floor of 3, and `data/predictions/scores.json` independently reports `ranked==True` on 28 of 45 with 189 scored predictions. Before round 2 that was 15 ranked and 117 scored. Nobody was demoted. Pipeline taxonomy, reported rather than summarised as a count: market_consensus attempted 226, `no_match` 202, `unavailable` 21, `failed` 3 (all `cli_nonzero_exit`); aggregate read 843 files and 2018 records and accepted 699. `scores.json` is now TRACKED with real provenance, where the plan's Safety section had recorded it as untracked and owned by repo-2 | `python3 -c "import json;d=json.load(open('data/predictions/scores.json'));L=d['leaders'];print(sum(1 for r in L if r.get('ranked')),'of',len(L))"` (expect `28 of 45`); `curl -s https://verbatim-predictions.tonygwu.com/ | grep -c 'less than 60 days before its own deadline'` (expect 1) |
