...
| CI-14 | Second Gemini account via the `tonyagents` macOS user | **attempted, mechanism verified**: `user:` profile kind and the `launchctl asuser` wrapper shipped with 30 checks. A real call as `tonyagents` answered and its log named its own Keychain. **blocked-on-two operator steps**: install the wrapper plus its sudoers rule, and re-login `agy` under `tonygwu` as tonygwu@gmail.com, because that Keychain still holds gptwufamily and both accounts are currently the same one. | `sudo -n /usr/local/libexec/agy-as-user tonyagents /usr/local/bin/agy --help` runs without a prompt; then each profile's identity differs |
